...
class BackgaugeSchematic(ctk.CTkFrame):
    ...
    def map_depth(self, value: float, x0: float, x1: float) -> float:
        if self.depth_max <= 0:
            return x0
        return x0 + (max(0.0, min(self.depth_max, value)) / self.depth_max) * (x1 - x0)

    def map_height(self, value: float, y0: float, y1: float) -> float:
        if self.height_max <= 0:
            return y1
        return y1 - (max(0.0, min(self.height_max, value)) / self.height_max) * (y1 - y0)
    ...
    def redraw(self):
        ...
        # bed and simple pseudo-isometric frame
        bed_y = bottom - 28
        ...
        # vertical reference / tooling area
        ref_x = left + 70
...
class AxisPanel(ctk.CTkFrame):
    def __init__(self, master, axis: AxisState, status_callback, change_callback=None):
        super().__init__(master)
        self.axis = axis
        ...
        # At Position indicator (right side of title)
        ...
        self.current_label = ctk.CTkLabel(self, text=self.format_value(axis.current), font=CURRENT_FONT, text_color="green")
        ...
        self.commanded_entry.insert(0, axis.commanded)
        ...
        for idx, step in enumerate(axis.jog_steps):
            ctk.CTkButton(
                ...
                text=f"+{step:0.3f}",
                height=SMALL_BUTTON_HEIGHT,
                command=lambda value=step: self.jog(value),
            ).grid(row=1, column=idx, sticky="ew", padx=6, pady=6)
        ...
        for idx, step in enumerate(axis.jog_steps):
            ctk.CTkButton(
                ...
                height=SMALL_BUTTON_HEIGHT,
                command=lambda value=step: self.jog(-value),
            ).grid(row=3, column=idx, sticky="ew", padx=6, pady=(6, 10))
    ...
    def update_colors(self):
        try:
            commanded = float(self.commanded_entry.get())
        except ValueError:
            return

        tol = 0.0005
        at_home = abs(self.axis.current - self.axis.min_limit) < tol
        in_position = abs(self.axis.current - commanded) < tol
    ...
    def set_commanded(self, value: float) -> None:
        ...
        self.commanded_entry.insert(0, self.format_value(value))
        self.update_colors()
        self.status_callback(f"{self.axis.name}: preset loaded ({self.format_value(value)})")
        if self.change_callback:
            self.change_callback()

    def clear_commanded(self) -> None:
        self.commanded_entry.delete(0, "end")
        self.commanded_entry.insert(0, "0.000")
        self.update_colors()
    ...
    def move_to_commanded(self) -> None:
        try:
            ...
            self.status_callback(f"{self.axis.name}: invalid commanded value")
            return

        value = max(self.axis.min_limit, min(self.axis.max_limit, value))
        self.axis.current = value
        self.current_label.configure(text=self.format_value(self.axis.current))
        self.update_colors()
        self.status_callback(f"{self.axis.name}: simulated move to {self.format_value(value)}")
        if self.change_callback:
            self.change_callback()

    def jog(self, delta: float) -> None:
        new_value = self.axis.current + delta
        new_value = max(self.axis.min_limit, min(self.axis.max_limit, new_value))
        self.axis.current = new_value
        self.current_label.configure(text=self.format_value(self.axis.current))
        self.commanded_entry.delete(0, "end")
        self.commanded_entry.insert(0, self.format_value(self.axis.current))
        self.update_colors()
        self.status_callback(f"{self.axis.name}: jogged to {self.format_value(self.axis.current)}")
        if self.change_callback:
            self.change_callback()
...
class HomePanel(ctk.CTkFrame):
    def __init__(self, master, home_all, home_depth, home_height):
        ...
        ctk.CTkButton(self, text="Home All", height=BUTTON_HEIGHT, command=home_all).grid(
            row=0, column=0, sticky="ew", padx=8, pady=8
        )
        ctk.CTkButton(self, text="Home Depth", height=BUTTON_HEIGHT, command=home_depth).grid(
            row=0, column=1, sticky="ew", padx=8, pady=8
        )
        ctk.CTkButton(self, text="Home Height", height=BUTTON_HEIGHT, command=home_height).grid(
...
class BackgaugeApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("TANN Backgauge Control - UI Skeleton")
        ...
        self.calculator = CalculatorPanel(
            self.left_panel,
            on_send_depth=self.send_calculator_to_depth,
            on_send_height=self.send_calculator_to_height,
        )
        ...
        self.depth_panel = AxisPanel(self, self.depth_axis, self.set_status, self.sync_schematic)
        self.depth_panel.grid(row=0, column=1, sticky="nsew", padx=10, pady=PANEL_PADY)

        self.height_panel = AxisPanel(self, self.height_axis, self.set_status, self.sync_schematic)
    ...
    def set_status(self, message: str) -> None:
        self.status_label.configure(text=f"SIM MODE  |  {message}")

    def sync_schematic(self) -> None:
        try:
            depth_commanded = float(self.depth_panel.commanded_entry.get())
        except ValueError:
            depth_commanded = self.depth_axis.current
        try:
            height_commanded = float(self.height_panel.commanded_entry.get())
        except ValueError:
            height_commanded = self.height_axis.current

        in_position = (
            abs(self.depth_axis.current - depth_commanded) < 0.0005
            and abs(self.height_axis.current - height_commanded) < 0.0005
        )
        self.schematic.set_values(
            self.depth_axis.current,
            depth_commanded,
            self.height_axis.current,
            height_commanded,
            in_position,
        )

    def send_calculator_to_depth(self, value: float) -> None:
        self.depth_panel.set_commanded(value)
        self.sync_schematic()

    def send_calculator_to_height(self, value: float) -> None:
        self.height_panel.set_commanded(value)
        self.sync_schematic()

    def home_depth(self) -> None:
        self.depth_axis.current = self.depth_axis.min_limit
        self.depth_panel.current_label.configure(text=self.depth_panel.format_value(self.depth_axis.current))
        self.depth_panel.set_commanded(self.depth_axis.current)
        self.sync_schematic()
        self.set_status("Stop Depth: homed")

    def home_height(self) -> None:
        self.height_axis.current = self.height_axis.min_limit
        self.height_panel.current_label.configure(text=self.height_panel.format_value(self.height_axis.current))
        self.height_panel.set_commanded(self.height_axis.current)
        self.sync_schematic()
        self.set_status("Stop Height: homed")
